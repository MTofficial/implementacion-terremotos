...
function App() {
  ...
  const fetchData = async (magnitudMinima: number, dias: number) => {
    try {
      ...
      const [sismosRes, statsRes] = await Promise.all([
        axios.get<Sismo[]>(
          `http://localhost:3000/api/sismos?minMag=${magnitudMinima}&dias=${dias}`,
        ),
        axios.get<Stats>(
          `http://localhost:3000/api/stats?minMag=${magnitudMinima}&dias=${dias}`,
        ),
      ]);
  ...
  useEffect(() => {
    fetchData(filtroMag, filtroDias);
  }, [filtroMag, filtroDias]);
  ...
  const handleSync = async () => {
    ...
    setErrorMsg(null);
    try {
      await axios.get("http://localhost:3000/api/sync");

      await fetchData(filtroMag, filtroDias);
    } catch (err) {
      console.error("Error al sincronizar:", err);
      setErrorMsg(
        "Ocurrió un error al intentar sincronizar con la API de Boostr.",
      );
    } finally {
      setIsSyncing(false);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-12 font-sans text-slate-800">
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              Monitoreo y estadísticas de la actividad sísmica reciente
            </p>
            <a
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              ...
              <p
                className="text-lg font-bold text-slate-700 mt-2 truncate"
                title={stats.sismoMasFuerte}
              >
        ...
        {/* Barra de Filtros */}
        {/* Barra de Filtros */}
        <div className="mb-8 flex flex-col md:flex-row items-center gap-6 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          <div className="flex items-center w-full md:w-auto">
            <label
              htmlFor="magFilter"
              className="font-semibold text-slate-700 mr-4 whitespace-nowrap"
            >
              Magnitud Mínima:
            </label>
            <select
              ...
              className="border border-slate-300 rounded-lg p-2 bg-slate-50 text-slate-700 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer w-full"
            >
